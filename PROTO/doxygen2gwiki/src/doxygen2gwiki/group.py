...
class DoxygenGroup:
    ...
    def createFiles(self):
        ...
        for b in self.detailed:
            b.getLines(detailed)
        return [("wiki", options.prefix + "_" + self.id, Group(searchList={"summary": "Documentation for the %s group" % (self.name, ), "ref": self.id, "labels": options.labels, "prefix": options.prefix, "filename": self.name, "briefdescription": "".join(brief).strip(), "detaileddescription": "".join(detailed).strip(), "functions": self.functions}))]
