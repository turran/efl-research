...
eina_tree_new(Eina_Compare_Cb compare_func)
{
	...
	new_tree = EINA_TREE(malloc(sizeof(Eina_Tree)));
	if (!new_tree)
	return NULL;
 ...
 */
EAPI int eina_tree_init(Eina_Tree *new_tree, Eina_Compare_Cb compare_func)
{
...
eina_tree_node_new()
{
	...
	new_node = EINA_TREE_NODE(malloc(sizeof(Eina_Tree_Node)));
	if (!new_node)
	return NULL;
...
EAPI int eina_tree_node_destroy(Eina_Tree_Node *node,
		Eina_Free_Cb value_free, Eina_Free_Cb key_free)
{
 ...
 * @return Returns TRUE on success, FALSE on failure.
 */
EAPI int eina_tree_for_each_node(Eina_Tree *tree,
		Eina_For_Each for_each_func, void *user_data)
{
	CHECK_PARAM_POINTER_RETURN("tree", tree, FALSE);
	CHECK_PARAM_POINTER_RETURN("for_each_func", for_each_func, FALSE);
...
static Eina_Tree_Node * tree_node_find_parent(Eina_Tree *tree,
		const void *key)
{
 ...
 */
static int tree_for_each_node(Eina_Tree_Node * node,
		Eina_For_Each for_each_func, void *user_data)
{
	CHECK_PARAM_POINTER_RETURN("node", node, FALSE);
